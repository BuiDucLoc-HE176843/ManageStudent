    ...
    public class AttendanceModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync(int classId)
        {
            ...
            foreach (var session in allSessions)
            {
                ...
                if (attendance != null)
                {
                    // Nếu có thông tin điểm danh nhưng IsPresent == null, hiển thị "Not Yet"
                    string status = attendance.IsPresent == null ? "Not Yet"
                                    : attendance.IsPresent == 1 ? "Present"
                                    : "Absent";
            ...
            // Kiểm tra tỷ lệ nghỉ
            AbsenceMessage = AbsencePercentage > 20
                ? "Bạn đã nghỉ quá 20% và sẽ không được qua môn này."
                : $"Tỷ lệ nghỉ: {AbsencePercentage:F2}%";
